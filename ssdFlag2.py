...
class SouthSudanFlag(Scene):
    def construct(self):
        ...
        black_stripe = Rectangle(width=flag_width, height=main_stripe_height).set_fill(BLACK, 1).set_stroke(width=0)
        black_stripe.shift(UP * (main_stripe_height + white_stripe_height))

        white_stripe1 = Rectangle(width=flag_width, height=white_stripe_height).set_fill(WHITE, 1).set_stroke(width=0)
        white_stripe1.shift(UP * (main_stripe_height/2 + white_stripe_height/2))

        red_stripe = Rectangle(width=flag_width, height=main_stripe_height).set_fill(RED, 1).set_stroke(width=0)
        
        white_stripe2 = Rectangle(width=flag_width, height=white_stripe_height).set_fill(WHITE, 1).set_stroke(width=0)
        white_stripe2.shift(DOWN * (main_stripe_height/2 + white_stripe_height/2))

        green_stripe = Rectangle(width=flag_width, height=main_stripe_height).set_fill(GREEN, 1).set_stroke(width=0)
        ...
        # Create the title text and position it under the flag
        title = Text(
            "South Sudan! We love and cherish you",
            font_size=36,
            color=BLACK
        )
        title.next_to(group, DOWN, buff=0.5)
