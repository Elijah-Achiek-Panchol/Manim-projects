...
class SouthSudanFlag(Scene):
    def construct(self):
        ...
        main_stripe_height = flag_height / 3.5  # Adjusted for white stripes
        white_stripe_height = main_stripe_height / 2

        # Total height calculation (3 main + 2 white stripes)
        total_height = (
            3 * main_stripe_height + 2 * white_stripe_height
        )
        # Scale factor to match original flag height
        scale_factor = flag_height / total_height

        # Draw stripes (order: black, white, red, white, green)
        # Black stripe
        ...
        # Blue triangle (adjusted for new stripe heights)
        triangle = Polygon(
            [-flag_width/2, total_height/2, 0],  # top-left
            [-flag_width/2, -total_height/2, 0], # bottom-left
            [-flag_width/2 + total_height * 0.8, 0, 0]  # right point (scaled)
